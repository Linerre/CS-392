...
int udp_msg_client(char *ip, int port, char *message, char *buf, int *buflen) {
    ...
    if (sendto(sockfd, message, message_length, 0, (struct sockaddr *)&si_other, si_len) == -1) {
        close(sockfd);
    ...
    int r;
    if ((r = recvfrom(sockfd, buf, *buflen, 0, (struct sockaddr *)&si_other, &si_len)) == -1) {
        close(sockfd);
        perror("Error: cound not receive from the server\n");
        exit(1);
    }
    buflen = &r;
